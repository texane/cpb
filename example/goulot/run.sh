...
# get_working_cpus 7;
# 33 the master
# WORKING_CPUS="$WORKING_CPUS 25 29 37 41 45"

for worksize in `seq 0 1 4` `seq 5 5 50`; do
    WORKING_CPUS='';

    echo -n "$worksize " ;

    # for hi in `seq 0 5`; do
    for hi in `seq 0 0`; do
	#get_working_cpus $hi ;
	WORKING_CPUS="0"
	LD_LIBRARY_PATH=/home/lementec/install/lib \
	    ...
	    ./main_$worksize $SOURCE_CPU $MASTER_CPU $WORKING_CPUS ;
	echo -n ' ';
	done ; # hi
    echo ;
done ; # worksize
